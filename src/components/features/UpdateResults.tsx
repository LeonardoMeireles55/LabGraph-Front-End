...
const UpdateResults: React.FC<{ analyticsType: string }> = ({ analyticsType }) => {
  ...
  return (
    <div className="flex items-center mr-4 gap-0 text-textSecondary">
      ...
      <label
        htmlFor="fileInput"
        className={`bg-background border border-borderColor text-textSecondary rounded px-2 py-1 text-base cursor-pointer hover:scale-110 ${status.isProcessing ? 'cursor-not-allowed opacity-50' : ''}`}
      >
