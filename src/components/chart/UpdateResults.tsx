...
const UpdateResults: React.FC<{ analyticsType: string }> = ({ analyticsType }) => {
  ...
  // Função para enviar os dados processados
  ...
  return (
    <div className="flex items-center mr-4 gap-0 text-textSecondary">
      ...
      <label
        htmlFor="fileInput"
        className={`bg-muted text-textSecondary rounded px-2 py-1 text-sm cursor-pointer hover:bg-accent ${status.isProcessing ? 'cursor-not-allowed opacity-50' : ''}`}
      >
