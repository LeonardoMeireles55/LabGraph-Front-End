...
const NavBar: React.FC<NavBarProps> = ({ jsonData, fileName }) => {
  ...
  return (
    <nav className="fixed left-0 top-0 z-50 w-full bg-navbar shadow-xl shadow-overlay transition-theme">
          ...
          <div className="hidden items-center gap-4 lg:flex xl:gap-6">
            {navLinks.map((link, index) => (
              <a
                key={index}
                href={link.url}
                className="group relative text-sm font-normal text-textPrimary transition-theme-fast hover:text-textPrimary xl:text-base"
              >
                {link.text}
                <span className="absolute bottom-0 left-0 h-0.5 w-0 bg-primary transition-theme-fast group-hover:w-full"></span>
              </a>
            ))}
      ...
      <div
        className={`fixed left-0 right-0 top-16 bg-surface shadow-lg shadow-shadow transition-theme sm:top-20 lg:hidden ${isMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'
          }`}
      >
